...
export class DrawService extends Service {
  ...
  public run(): boolean {
    ...
    this.execProcess = spawn("node", ["../Draw/dist/index.js"], {
      env: process.env,
      stdio: [null, 'inherit','inherit', 'ipc']  
    });

    // this.execProcess = spawn("ts-node", ["../Draw/src/index.ts"], {
    //   env: process.env,
    //   stdio: [null, 'inherit','inherit', 'ipc']  
    // });
  ...
  public send(msg: string) {
    //return;
    console.log("ServiceManager Service got message, trying to send to Service:", msg)
    this.execProcess.send(msg)
  }
}
